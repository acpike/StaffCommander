...
- `v0.15-desert-cacti` — saguaro cacti + boulders + mesas for Desert Run (lean: shared geo, no shadows). Verified on screen; build green.
- `v0.16-qa-fixes` — QA review-agent pass: fixed React-19 ref-callback, deleted-custom-level fallback, Track texture disposal (leak), stale unlock copy. QA confirmed store logic (mastery/gems/achievements/daily/migrations) sound. Build green, verified. (Minor follow-ups: space/planet texture disposal; mountain/desert ref naming cosmetic.)
